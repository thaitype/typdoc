...
- `hostname` 0.4, default features off (only `get` is used, never `set`), a dependency of the binary crate only: the lock file stamps the machine's hostname, and the standard library has no call for it; this is the smallest, dependency-light crate that reads it through the ordinary system call rather than shelling out to `hostname(1)`
