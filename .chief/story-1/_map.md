...
- The design does not say how a `state/<namespace>.json` that does not exist yet is treated (a fresh namespace has none, and `typdoc new` is what writes it). Raised while writing ticket 9's write-command tests, which cannot assert it until the design does.
