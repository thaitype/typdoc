...
2. **The rules the binary has.** The registry holds every rule in the design's tables except `frontmatter.transitions`, which is in `unimplemented_rules`. Default: the same two checks as for commands, extended to rules, because the coverage checks of ticket 9 compare the design with the registry for commands only, so a rule that the design lists and nobody builds would not be noticed.
...
4. **Nothing is written.** `typdoc-core` calls no function that changes the file system. A `clippy.toml` in its directory lists `std::fs::write`, `rename`, `copy`, `remove_file`, `create_dir`, `create_dir_all`, `std::fs::File::create` and `std::fs::File::create_new` as disallowed methods, and story 1 has no exception to it. That makes "the read core changes nothing" a property of the code and not a matter of care. A test that needs a file uses a fixture in the repository. Not verified: that `remove_file`, `create_dir` and `create_dir_all` work as disallowed methods; the others were run (ticket 9).
...
8. **The accounting invariant.** In `--audit --json`, for every project in the fixtures and for the two acceptance runs named in the goal, `summary.checked.documents` plus `summary.unreported.uncollected` plus `summary.unreported.no_frontmatter` equals the number of `.md` files the run reads, counted independently of typdoc. A document whose block cannot be parsed is checked, since it has a finding, and is not counted as unreported.
