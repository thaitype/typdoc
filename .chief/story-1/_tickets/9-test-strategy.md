...
Decided: three layers, each closing a gap the others leave. A forced kill and a power loss leave a stale lock by design (ticket 7), so the signal handler is best effort and no test claims more than that.
...
2. **The shipped binary, with no hook.** The test creates the lock file itself, then runs the shipped binary on a command that needs that lock. The binary cannot take it and waits, which is a known state, long enough and fully determined, and needs nothing added to the binary. The test sends SIGINT (and, in a second run, SIGTERM) while it waits. It asserts that the process exits by its own exit and is not killed by the signal, and that the test's lock file is still there with the same device and inode (a process that never held the lock removes nothing). This is the only layer that proves a handler is installed in the build a user receives.
3. **A test build with a pause point.** A Cargo feature, absent from the shipped build, stops the process just after it has taken the lock, so a signal arrives while a lock really is held. It asserts that the lock file is gone afterwards and that the exit status is the one the design states. It proves the cleanup, and only the cleanup.

The gaps and how they are closed: layer 3 runs a build that differs from the shipped one, so it cannot show that the shipped build installs a handler; layer 2 shows that. Layer 2 never holds a lock, so it cannot show cleanup; layer 3 shows that. Layer 1 needs no process and covers the identity check that decides whether to remove. This is the same shape as the three layers for remote schemas (a fake, the real adapter, the real binary), not a new one.

Rejected: sending signals at random moments and counting how many rounds prove enough. Making a proof depend on a number nobody derived is the mistake this ticket avoids elsewhere, and a signal test that goes red now and then ends up switched off, not investigated.

The tests observe results only: whether the lock file exists, its device and inode, whether the process exited or was killed, and its exit code. None looks at how the handler is built, because the crate that catches signals is chosen in the contract (ticket 7) and any choice must leave these tests valid.

Defaults: each layer is shown able to fail, as the text gate and the guard on `deps` were. Layer 2 goes red when the binary is built with no handler (the process is killed by the signal). Layer 3 goes red when the handler skips the removal. Layers 1 and 2 go red together when the cleanup removes by path without the identity check. Test processes are started directly and never as a background job of a shell script: a non-interactive bash starts background jobs with SIGINT ignored, so a signal sent to one does nothing.

Probe of 2026-09-20 (Linux, Python 3.14.4 `subprocess`, GNU bash 5.3.9): a child with no handler that is killed by SIGINT reports return code -2, killed by signal; a child whose handler exits by itself with 7 reports 7; a child whose handler restores the default and raises the signal again reports -2, exactly as if it had no handler; bash reports 130 for a foreground child killed by SIGINT; `sleep 5 &` in a non-interactive bash ignored SIGINT and finished normally. Not verified: SIGTERM, macOS.

Not decided here, and needed before layer 2 can tell the two cases apart: what a process that is interrupted exits with. The probe shows that a handler which cleans up and then raises the signal again looks the same as no handler, so layer 2 works only if the design says the process leaves by its own exit and gives the code. The exit table stops at 4 and project rule 5 says codes 0 to 4, so any code for an interrupt is a change to one or the other.
