...
Decided: Linux and macOS only in v1. On Windows the build fails with a clear message (`compile_error!`); WSL is Linux and works. Amended by ticket 9: the code still targets both, but macOS is not yet a supported platform, because no run on macOS has shown it. Keys and refs always use `/`, so a repository can be shared across operating systems later. macOS is claimed only when a real macOS CI run checks it (ticket 9).
...
1. The list contains only shells that have actually been run. A row is a claim; a row for a shell that was never run describes an intention that a reader takes for a state. So the v1 rows are sh and bash, each backed by a test that runs through the real shell. zsh is not one of them: it is not installed on the machine where this was decided, so under this rule its row is removed. zsh is macOS's default shell and macOS is a target, so the most common way to run typdoc on macOS is not on the list that has evidence; that is a known gap. Quoting is behaviour of the shell, not of the operating system, so installing zsh on a Linux machine and running the test would be full evidence for zsh on macOS, unlike the claim about macOS itself, which needs a macOS runner. The row returns together with a test that runs zsh, never on its own.
2. The behaviour of an untested shell is not described. fish's handling of a single-quoted `\\` comes from its documentation and was never run, so it stays out of the document. The text says only that zsh, fish, PowerShell and cmd are not covered in v1.
...
Not verified: run on this machine were bash and dash (as `sh`), where single quotes kept `\*`, `\,` and `\\` intact. Not run: zsh (not installed), fish, PowerShell and cmd. zsh therefore has no evidence and is not on the list (see 1). The design states the rule (a shell is listed only while a test runs it); what the suite does when a listed shell is missing is decided in ticket 9.
