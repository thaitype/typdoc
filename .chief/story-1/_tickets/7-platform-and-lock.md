...
1. The list contains only shells that have actually been run. A row is a claim; a row for a shell that was never run describes an intention that a reader takes for a state. So the v1 rows are sh and bash, each backed by a test that runs through the real shell. zsh is not one of them: it is not installed on the machine where this was decided, so under this rule its row is removed. zsh is macOS's default shell and macOS is supported, so the most common way to run typdoc on macOS is not on the list that has evidence; that is a known gap. Quoting is behaviour of the shell, not of the operating system, so installing zsh on a Linux machine and running the test would be full evidence for zsh on macOS, unlike the claim about macOS itself, which needs a macOS runner. The row returns together with a test that runs zsh, never on its own.
