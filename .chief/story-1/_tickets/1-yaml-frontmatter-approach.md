...
**Recommendation: split reading from writing.**
...
- **Write** with `yaml-edit`, pinned to an exact 0.3.x, restricted to three operations: set a scalar, append or remove a list item, add a key. It is the only crate that preserved comments, quote style and flow style in the researcher's probes, with a byte-identical no-op round trip. Any parse-then-reserialise write (every serde-family crate) drops comments, quotes and flow style.
...
- **`serde_yaml` is archived**, confirmed independently on crates.io (latest `0.9.34+deprecated`, last updated 2024-03-25).
...
**Risk to weigh:** `yaml-edit` 0.3.2 was published on 2026-09-17, two days before this research (crates.io, checked independently). It is young and single-maintainer; the exact-version pin and the reparse guard are what make that acceptable, and the guard is the reason the runner-up remains a credible fallback.

## Not verified

No YAML 1.1 parser was run; `serde_yaml_ng` and `serde_norway` were not run; the cause of `yamlpatch`'s flow-list replace failure is unknown; `saphyr` 0.1.0 span bugs were observed but not checked upstream; `yaml-edit` was not tested on anchors or tags. A research recommendation, not yet a human decision: it stands unless the human overrides it.
