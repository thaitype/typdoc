...
`body-type` frontmatter field, and:
- if `body-type` is `json`, parses the body as JSON and deserializes it into a caller-given type
  via `serde`;
- if `body-type` is missing, unrecognized, or the body doesn't parse as valid JSON when it says
...
Dispatch is entirely on the `body-type` field; nothing about the function's behavior depends on
...
- A catalog document missing `body-type` fails with its own distinct error.
- A catalog document with an unrecognized `body-type` value fails with its own distinct error.
- A catalog document whose body is not valid JSON, despite declaring `body-type: json`, fails
