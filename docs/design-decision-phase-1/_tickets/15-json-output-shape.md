...
Still to decide in this ticket: which config errors appear as findings in the `validate` report and which stop the command with an error object, the shape of the summary, and the shapes and order of the arrays of `toc`, `refs` and `validate`.
