...
- Defaults: the re-read uses the reader (`yaml_serde`), not the writer's own parser, so the two crates must agree; fixtures are committed to the public repo only after review (this repo's own files carry no frontmatter, since its tickets use inline lines, so real documents come from the public `chief` and `typmem` repositories after review and the public-text gate; otherwise synthetic files with the same shape), and a test never skips silently when a file is missing.
