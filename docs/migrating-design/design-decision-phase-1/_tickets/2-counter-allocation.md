...
- **`last` is the never-reuse guarantee.** Delete `T-12` and the highest remaining file is `T-11`, but `last` still says 12, so the next number is 13. Counting files alone would reuse 12, and a body link, commit message or chat message citing it would silently point at a different document. A file created by hand with a higher number is still respected; the two sources can only disagree by leaving a gap, which is harmless. Amended by ticket 9: the guarantee holds while the record exists. A record that is lost while coded documents remain is reported by the always-on rule `state.missing`, and `new` refuses to issue a number until it is restored.
