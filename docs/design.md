...
**Audit.** With `--audit` the report also has `audit`: `collections`, one `{ "name", "documents" }` for each collection with the number of documents it holds, sorted by name; `uncollected`, the `path` of every file that belongs to no collection; and `no_frontmatter`, the `path` of every file that belongs to a collection and has no frontmatter, both sorted as `path` is. The two lists do not overlap: a file in no collection has no schema to say what its frontmatter should hold, so it is only in `uncollected`. The documents per collection cannot be worked out from `findings`, since a clean document produces none, and a finding carries its `collection` so that the counts per collection and rule, which the text form prints as a table, can be worked out from `findings`; they are not in the JSON.
