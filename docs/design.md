...
.typdoc/
  ...
  locks/         one mutex file per namespace     .gitignore
```
...
**Machine-specific imports.** When an imported project's location differs per machine, put it in an environment variable or in `~/.config/typdoc/imports.json`, which is merged under the project's own `imports`, so no machine-specific path is committed.
...
| `imports.absent` | `warn` | — | Refs into an imported project that is absent on this machine |
