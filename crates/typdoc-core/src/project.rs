...
use crate::fs::{create_exclusively, write_atomically};
...
/// a destination that is already there into [`Error::Exists`] with `message` rather than the
/// bare `io::Error` `create_exclusively` returns (decision 15) — the one difference between the
/// two forms at this last step is what `message` says, since a coded form's collision names the
/// key it had just allocated and a path form's names the path the caller gave.
